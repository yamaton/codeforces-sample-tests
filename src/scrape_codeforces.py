...
@author yamaton
@date 2015-08-31
      2018-11-27 Problem number can be four digits.
"""
...
def is_proper(prob_id):
    ...
    if prob_id != decimals + letters or len(letters) != 1 or len(decimals) > 4:
        return False
    return True
...
def tag_content(bsform, classname, tag="div"):
    ...
    return [[x for x in xs if type(x) is bs4.element.NavigableString] for xs in lst]
